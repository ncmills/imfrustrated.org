...
import { getParkingCity, getParkingDefense } from "@/data/parking";

export const runtime = "edge";
export const alt = "Parking ticket defense — I'm Frustrated dot Org";
...
export default async function Image({
  ...
  params: Promise<{ city: string; defense: string }>;
}) {
  const { city, defense } = await params;
  const c = getParkingCity(city);
  const d = c ? getParkingDefense(city, defense) : undefined;

  const title = d?.title ?? "Parking Ticket Defense";
  const categoryLabel = c ? `${c.state} · Parking` : "Parking";
    ...
    (
      ...
      >
        ...
        >
          ...
          >
            imfrustrated.org/letters/parking
          </span>
        </div>
      </div>
    ),
